void replay_boiling_shms (Int_t RunNumber = 0, Int_t MaxEvent = 0) {
  ...
  const char* ROOTFileNamePattern = "ROOTfiles/CALIB/shms_replay_boiling_%d_%d.root";
  ...
  */
  ...
  // Define DEF-file
  analyzer->SetOdefFile("DEF-files/SHMS/CALIBRATION/pboil_calibration.def");
  // Define cuts file
  analyzer->SetCutFile("DEF-files/SHMS/PRODUCTION/CUTS/pstackana_production_cuts.def");  // optional
  // File to record accounting information for cuts
  //analyzer->SetSummaryFile(Form("", RunNumber, MaxEvent));  // optional
  // Start the actual analysis.
  analyzer->Process(run);
  // Create report file from template
  //analyzer->PrintReport("",Form(""));  // optional

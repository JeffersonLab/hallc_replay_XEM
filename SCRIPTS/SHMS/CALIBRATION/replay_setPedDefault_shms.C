...
void replay_setPedDefault_shms (Int_t RunNumber = 0, Int_t MaxEvent = 0) {
  ...
  const char* ROOTFileNamePattern = "ROOTfiles/SHMS/CALIBRATION/shms_setPedDefault_%d_%d.root";
  TString ROOTFileName = Form(ROOTFileNamePattern, RunNumber, MaxEvent);
  //Specifics for the replay
  TString odef_file = "DEF-files/SHMS/PRODUCTION/SHMS_PedDefault.def";
  TString cdef_file = "DEF-files/SHMS/PRODUCTION/CUTS/pstackana_production_cuts.def";
  TString summary_file = Form("REPORT_OUTPUT/SHMS/PRODUCTION/summary_shms_setPedDefault_%d_%d.report",
			      RunNumber, MaxEvent);
  TString report_file  = Form("REPORT_OUTPUT/SHMS/CALIBRATION/replay_shms_setPedDefault_%d_%d.report",
			      RunNumber, MaxEvent);

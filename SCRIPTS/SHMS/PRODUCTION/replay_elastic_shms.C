...
void replay_elastic_shms (Int_t RunNumber = 0, Int_t MaxEvent = 0) {
  ...
  const char* ROOTFileNamePattern = "ROOTfiles/SHMS/HeeP/shms_replay_production_%d_%d.root";
  TString ROOTFileName = Form(ROOTFileNamePattern, RunNumber, MaxEvent);
  //Specifics for the replay
  TString odef_file = "DEF-files/SHMS/PRODUCTION/shms_elastic.def";
  TString cdef_file = "DEF-files/SHMS/PRODUCTION/CUTS/pstackana_production_cuts.def";
  TString summary_file = Form("REPORT_OUTPUT/SHMS/HeeP/summary_production_%d_%d.report",
			      RunNumber, MaxEvent);
  TString report_file  = Form("REPORT_OUTPUT/SHMS/HeeP/replay_shms_production_%d_%d.report",
			      RunNumber, MaxEvent);

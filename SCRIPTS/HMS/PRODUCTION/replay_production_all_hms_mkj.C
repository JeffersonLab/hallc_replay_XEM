...
void replay_production_all_hms_mkj (Int_t RunNumber=0, Int_t MaxEvent=0) {
  ...
  const char* ROOTFileNamePattern = "ROOTfiles/mkj_hms_replay_production_all_%d_%d.root";
  TString ROOTFileName = Form(ROOTFileNamePattern, RunNumber, MaxEvent);
  //Specifics for the replay
  TString odef_file = "DEF-files/HMS/PRODUCTION/mkj_hstackana_production_all.def";
  TString cdef_file = "DEF-files/HMS/PRODUCTION/CUTS/hstackana_production_cuts.def";
  TString summary_file = Form("REPORT_OUTPUT/HMS/PRODUCTION/mkj_summary_all_production_%d_%d.report", RunNumber, MaxEvent);
  TString report_file  = Form("REPORT_OUTPUT/HMS/PRODUCTION/Mkj_replay_hms_all_production_%d_%d.report", RunNumber, MaxEvent);

  //Initialize gHcParms.
  //Shared HMS gHcParms setup located in ../hms_shared.h
  setupParms(RunNumber);
  gHcParms->Load("PARAM/HMS/GEN/mkj.param");

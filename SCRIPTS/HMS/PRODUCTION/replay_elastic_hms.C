...
void replay_elastic_hms(Int_t RunNumber=0, Int_t MaxEvent=0) {
  ...
  const char* ROOTFileNamePattern = "ROOTfiles/HMS/HeeP/hms_replay_production_%d_%d.root";
  TString ROOTFileName = Form(ROOTFileNamePattern, RunNumber, MaxEvent);
  //Specifics for the replay
  TString odef_file = "DEF-files/HMS/PRODUCTION/hms_elastic.def";
  TString cdef_file = "DEF-files/HMS/PRODUCTION/CUTS/hstackana_production_cuts.def";
  TString summary_file = Form("REPORT_OUTPUT/HMS/HeeP/summary_production_%d_%d.report",
			      RunNumber, MaxEvent);
  TString report_file  = Form("REPORT_OUTPUT/HMS/HeeP/replay_hms_production_%d_%d.report",
			      RunNumber, MaxEvent);

...
mkdir -p PARAM/HMS/BCM

# Directory containing the files
base_dir="/cache/hallc/xem2/analysis/OFFLINE/REPLAYS/HMS/PRODUCTION/pass2"

# Loop over all files matching the pattern
for file in "$base_dir"/hms_replay_production_*_-1.root; do
  # Extract the run number from the file name
  if [[ $file =~ hms_replay_production_([0-9]+)_-1\.root ]]; then
    run="${BASH_REMATCH[1]}"

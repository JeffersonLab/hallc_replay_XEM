...
base_dir="/cache/hallc/xem2/analysis/OFFLINE/REPLAYS/SHMS/PRODUCTION/pass2.1"

# Loop over all files matching the pattern
for file in "$base_dir"/shms_replay_production_*_-1.root; do
  # Extract the run number from the file name
  if [[ $file =~ shms_replay_production_([0-9]+)_-1\.root ]]; then
    run="${BASH_REMATCH[1]}"
